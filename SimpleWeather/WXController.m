...
//  SimpleWeather
//
//  Created by Alejandro Juárez Robles on 5/9/14.
...
- (void)viewDidLoad
{
    ...
    [[RACObserve([WXManager sharedManager], currentBackgroundImage)
      deliverOn:RACScheduler.mainThreadScheduler]
     subscribeNext:^(WXBackgroundImage *bgImage) {
         if (bgImage != nil ) {
             dispatch_async(dispatch_get_global_queue(0,0), ^{
                 NSData *data = [[NSData alloc] initWithContentsOfURL: [NSURL URLWithString: bgImage.file_url]];
                 if ( data == nil )
                     return;
